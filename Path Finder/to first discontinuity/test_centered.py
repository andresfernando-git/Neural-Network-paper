...
for difsamples in range(1):
    ...
    reference_state_dict = torch.load('/home/andres/Videos/Karoline results/discontinuity1/discontinuity1(0.0248000003-0.0248000004).pth', map_location=device)
    reference_params = {name: param for name, param in reference_state_dict.items()}

    # Open metrics file and write header
    with open(f'Model-{difsamples}.csv', 'w') as f:
        ...
        for L2_strength in L2_strengths:
            ...
            with open(f'gradients-{difsamples}-{L2_strength:.5f}.csv', 'w') as grad_file:
                for name, param in model.named_parameters():
            ...
            torch.save(model.state_dict(), f'model_weights-{difsamples}-{L2_strength:.5f}.pth')
            ...
            with open(f'Hessian-eigenvalues-{difsamples}-{L2_strength:.5f}.csv', 'w') as eigval_file:
                np.savetxt(eigval_file, eigenvalues, delimiter='\n', fmt='%.9f')
            with open(f'Hessian-eigenvectors-{difsamples}-{L2_strength:.5f}.csv', 'w') as eigvec_file:
