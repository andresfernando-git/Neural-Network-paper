...
for difsamples in range(1):
    ...
    model = Net().to(device)
    model.load_state_dict(torch.load('model_weights-3-0.00000.pth', map_location=device))

    # Load reference parameters
    reference_state_dict = torch.load('model_weights-4-0.00000.pth', map_location=device)
    reference_params = {name: param for name, param in reference_state_dict.items()}

    # Open metrics file and write header
    with open(f'Model-{difsamples}-[3_to_4].csv', 'w') as f:
        f.write('L2_strength,Train_Loss,Train_Reg_Loss,Test_Loss,Test_Accuracy\n')

        # Loop over L2 regularization strengths
        L2_strengths = [i*0.00000001 for i in range(501)]
        for L2_strength in L2_strengths:
            ...
            f.write(f'{L2_strength:.9f},{train_loss:.9f},{train_reg_loss:.9f},{test_loss:.9f},{100*test_accuracy:.9f}\n')
            ...
            # Save the gradients to a text files
#            with open(f'gradients-{difsamples}-{L2_strength:.9f}-[0_to_4].csv', 'w') as grad_file:
#                for name, param in model.named_parameters():
...
#
            ...
            torch.save(model.state_dict(), f'model_weights-{difsamples}-{L2_strength:.9f}-[3_to_4].pth')
            ...
            with open(f'Hessian-eigenvalues-{difsamples}-{L2_strength:.9f}-[3_to_4].csv', 'w') as eigval_file:
                np.savetxt(eigval_file, eigenvalues, delimiter='\n', fmt='%.9f')
#            with open(f'Hessian-eigenvectors-{difsamples}-{L2_strength:.9f}-[1_to_2].csv', 'w') as eigvec_file:
